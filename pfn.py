from model import PureGCN_v1, PureGCN, PFNPredictor
...
def parse():
    ...
    parser.add_argument("--hidden", default=255, type=int)
    ...
    parser.add_argument('--mlp_res', type=str2bool, default=False)

    parser.add_argument('--transformer_hidden', type=int, default=16)
    parser.add_argument('--transformer_layers', type=int, default=1)
    parser.add_argument('--nhead', type=int, default=4)
    parser.add_argument('--context_num', type=int, default=128)
    parser.add_argument('--dynamic', type=str2bool, default=False)
    parser.add_argument('--padding', type=str, default='zeros')
    parser.add_argument('--premlp', type=str2bool, default=False)
    parser.add_argument('--output_target', type=str2bool, default=False)
    parser.add_argument('--column_att', type=str2bool, default=False)
...
def train(model, data, train_pos_edge, optimizer, pred, batch_size, maskinput, num_neg):
    ...
    for perm in dataloader:
        ...
        pos_edge = pos_edge.t().to(data.x.device)
        neg_edge = neg_edge.t().to(data.x.device)

        pos_score = pred(data, h, pos_edge)
        neg_score = pred(data, h, neg_edge)
...
def train_all(model, data_list, split_edge_list, optimizer, pred, batch_size, maskinput, num_neg):
    tot_loss = 0
...
@torch.no_grad()
def test(model, predictor, data, pos_test_edge, neg_test_edge, pos_valid_edge, neg_valid_edge, 
         pos_train_edge, evaluator, batch_size):
    st = time.time()
    model.eval()
    predictor.eval()
    ...
    train_pos_score = []
    for idx in pos_valid_loader:
        e = pos_train_edge[:, idx].to(data.x.device)
        out = predictor(data, h, e)
    ...
    for idx in pos_test_loader:
        e = pos_test_edge[:, idx].to(data.x.device)
        out = predictor(data, h, e)
    ...
    for idx in neg_test_loader:
        e = neg_test_edge[:, idx].to(data.x.device)
        out = predictor(data, h, e)
...
def test_all(model, predictor, data_list, split_edge_list, evaluator, batch_size, 
             metric='hits@100'):
    ...
    for data, split_edge in zip(data_list, split_edge_list):
        ...
        train_metric, valid_metric, test_metric = test(model, predictor, data, pos_test_edge, 
                                                       neg_test_edge, pos_valid_edge, neg_valid_edge, 
                                                       pos_train_edge, evaluator, batch_size)
...
def run(args):
    ...
    for data in data_list:
        ...
        data.context_pos = data.edge_index[:, torch.randperm(data.edge_index.size(1))[:args.context_num // 2]].to(device)
        data.context_neg = negative_sampling(data.edge_index, num_nodes=data.num_nodes, 
                                             num_neg_samples=args.context_num // 2).to(device) 
    ...
    if args.predictor == 'PFN':
        predictor = PFNPredictor(args.hidden, args.nhead, args.transformer_layers, args.mlp_layers, args.dp, 
                                 args.norm, args.scale, args.dynamic, args.context_num, args.padding, 
                                 args.premlp, args.output_target, args.column_att)
    else:
        raise NotImplementedError
    ...
    for epoch in range(args.epochs):
        ...
        loss = train_all(model, data_list, split_edge_list, optimizer, predictor, args.batch_size, 
                         args.maskinput, args.num_neg)
        train_metric, valid_metric, test_metric = test_all(model, predictor, data_list, split_edge_list, 
                                                           evaluator, args.test_batch_size, args.metric)
    ...
    train_metric, valid_metric, test_metric = test(model, predictor, data, pos_test_edge, 
                                                   neg_test_edge, pos_valid_edge, neg_valid_edge, 
                                                   pos_train_edge, evaluator, args.test_batch_size)
    print(f"Test time: {time.time()-st}", flush=True)
    ...
    '''for i in range(100):
        ...
        train_metric, valid_metric, test_metric = test(model, predictor, data, pos_test_edge,
                                                    neg_test_edge, pos_valid_edge, neg_valid_edge,
                                                    pos_train_edge, evaluator, args.test_batch_size)
        print(f'context_test: {test_metric[args.metric]}', flush=True)
        wandb.log({'context_test': test_metric[args.metric]})'''
