from model import PureGCN_v1, PureGCN, Hadamard_MLPPredictor
...
class NoiseAdapter(nn.Module):
    def __init__(self, in_feats, out_feats, norm=False):
        super().__init__()
        noise = torch.randn(in_feats, out_feats) / torch.sqrt(torch.tensor(out_feats).float())
...
def parse():
    ...
    parser.add_argument('--train_dataset', type=str, default='CiteSeer')
    ...
    parser.add_argument('--noise_norm', type=bool, default=False)
...
def run(args):
    ...
    for data in data_list:
        ...
        data.noise = NoiseAdapter(data.x.size(1), args.hidden, args.noise_norm).to(device)
        if 'full_adj_t' in data:
    ...
    optimizer = torch.optim.Adam(list(predictor.parameters() + model.parameters()), lr=args.lr)
    print(f'number of parameters: {sum(p.numel() for p in predictor.parameters()) + sum(p.numel() for p in model.parameters())}')
